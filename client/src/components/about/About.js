import './About.css'

export const About = () => {
  return (
    <div className='about-container'>
      <div className='about'>
        <div className='about__header-img' />
        <p className='tang-b gold'>About</p>
      </div>
      <div className='about__txt'>
        <p>꧁ Delivery of products within the continental United States.</p>
        <p>꧁ Returns and exchanges are not accepted related to infection control practices.</p>
        <p>꧁ Please contact me if you have any problems with your order.</p>
        <p>꧁ Shipping is priority mail with a 1-3 day delivery.</p>
        <p>꧁ All stocked merchandise ships in 2 business days.</p>
        <p>꧁ Contact us at thankyougorgeous1@gmail.com</p>
      </div>
    </div>
  )
}
